@php
    $volverRuta = $project ? route('proyectos.ver', $project) : route('proyectos');
@endphp
<div class="p-4 sm:p-6 lg:p-8">
    <div class="max-w-3xl mx-auto space-y-5 anim-fade-up">
        ...
        <x-page-header :title="$project ? 'Editar proyecto' : 'Nuevo proyecto'" subtitle="Datos del proyecto y su equipo" icon="folder">
            <x-slot:actions>
                <a href="{{ $volverRuta }}" wire:navigate class="inline-flex items-center gap-1 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition">
        ...
        <form wire:submit="save" class="rounded-2xl bg-white dark:bg-slate-900 shadow-sm dark:shadow-black/20 border border-slate-200/70 dark:border-slate-800 p-6 space-y-5">
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <a href="{{ $volverRuta }}" wire:navigate class="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition">Cancelar</a>
            </div>
        </form>
    </div>
</div>
