@if ($enModal)
    <div>
        <form wire:submit="save" class="space-y-6">
            @include('livewire.organization.departamentos.partials.campos-formulario')
...
@else
    <div class="p-4 sm:p-6 lg:p-8">
        <div class="max-w-2xl mx-auto space-y-5 anim-fade-up">
            ...
            <x-page-header :title="$department ? 'Editar departamento' : 'Nuevo departamento'" subtitle="Estructura organizacional" icon="building">
                <x-slot:actions>
                    <a href="{{ route('departamentos') }}" wire:navigate class="inline-flex items-center gap-1 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition">
            ...
            <form wire:submit="save" class="rounded-2xl bg-white dark:bg-slate-900 shadow-sm dark:shadow-black/20 border border-slate-200/70 dark:border-slate-800 p-6 space-y-6">
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('departamentos') }}" wire:navigate class="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
